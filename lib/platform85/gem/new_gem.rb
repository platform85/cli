...
module Platform85
  module Gem
    class NewGem
      ...
      attr_reader :options, :gem_name, :name, :target, :extension
      ...
      def initialize(gem_name, options)
        ...
        @name = @gem_name
        @target = Bundler::SharedHelpers.pwd.join(gem_name)
      ...
      def generate
        ...
        constant_name = name.gsub(/-[_-]*(?![_-]|$)/) do
                          "::"
                        end.gsub(/([_-]+|(::)|^)(.|$)/) { ::Regexp.last_match(2).to_s + ::Regexp.last_match(3).upcase }
        constant_array = constant_name.split("::")
        minitest_constant_name = # Foo::Bar => Foo::TestBar
          constant_array.clone.tap do |a|
            a[-1] = "Test#{a[-1]}"
          end.join("::")
        ...
        templates.each do |src, dst|
          ...
          template("newgem/#{src}", destination, config)
        end
        ...
        IO.popen(%w[git add .], { chdir: target }, &:read) if use_git
      ...
      def resolve_name(name)
        Bundler::SharedHelpers.pwd.join(name).basename.to_s
      end
      ...
      def ask_and_set_test_framework
        ...
        if test_framework.to_s.empty?
          ...
          test_framework = if /rspec|minitest|test-unit/.match?(result)
                             result
                           else
                             false
                           end
        ...
        Bundler.settings.set_global("gem.test", test_framework) if Bundler.settings["gem.test"].nil?
      ...
      def ask_and_set_ci
        ...
        if ci_template.to_s.empty?
          ...
          ci_template = if /github|gitlab|circle/.match?(result)
                          result
                        else
                          false
                        end
        ...
        Bundler.settings.set_global("gem.ci", ci_template) if Bundler.settings["gem.ci"].nil?
      ...
      def ask_and_set_linter
        ...
        if linter_template.to_s.empty?
          ...
          linter_template = if /rubocop|standard/.match?(result)
                              result
                            else
                              false
                            end
        ...
        Bundler.settings.set_global("gem.linter", linter_template) if Bundler.settings["gem.linter"].nil?

        # Once gem.linter safely set, unset the deprecated gem.rubocop
        Bundler.settings.set_global("gem.rubocop", nil) unless Bundler.settings["gem.rubocop"].nil?
      ...
      def travis_removal_info
        ...
        return unless Bundler.settings["gem.ci"] == "travis"
      ...
      def validate_rust_builder_rubygems_version
        return unless ::Gem::Version.new(rust_builder_required_rubygems_version) > ::Gem.rubygems_version

        Bundler.ui.error "Your RubyGems version (#{::Gem.rubygems_version}) is too old to build Rust extension. Please update your RubyGems using `gem update --system` or any other way and try again."
        exit 1
      end

      def template1(source, destination, config:)
